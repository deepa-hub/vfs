...
static int read_name(struct inode *ino, char *name)
{
	...
	ino->i_atime = timespec_to_vfs_time(st.atime);
	ino->i_mtime = timespec_to_vfs_time(st.mtime);
	ino->i_ctime = timespec_to_vfs_time(st.ctime);
	ino->i_size = st.size;
...
static int hostfs_setattr(struct dentry *dentry, struct iattr *attr)
{
	...
	if (attr->ia_valid & ATTR_ATIME) {
		...
		attrs.ia_atime = vfs_time_to_timespec(attr->ia_atime);
	}
	if (attr->ia_valid & ATTR_MTIME) {
		attrs.ia_valid |= HOSTFS_ATTR_MTIME;
		attrs.ia_mtime = vfs_time_to_timespec(attr->ia_mtime);
	}
	if (attr->ia_valid & ATTR_CTIME) {
		attrs.ia_valid |= HOSTFS_ATTR_CTIME;
		attrs.ia_ctime = vfs_time_to_timespec(attr->ia_ctime);
	}
