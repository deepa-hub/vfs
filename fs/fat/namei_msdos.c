			   ...
			   struct timespec *ts, struct fat_slot_info *sinfo)
{
	...
	dir->i_ctime = dir->i_mtime = timespec_to_vfs_time(*ts);
	if (IS_DIRSYNC(dir))
...
static int msdos_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	clear_nlink(inode);
	inode->i_ctime = current_fs_time_sec(sb);
	fat_detach(inode);
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	if (!err)
			   ...
			   struct dentry *new_dentry, int is_hid)
{
	...
	if (!err) {
		if (!new_inode) {
			...
			old_dir->i_ctime = old_dir->i_mtime =
				current_fs_time_sec(old_dir->i_sb);
			if (IS_DIRSYNC(old_dir))
	...
	ts = vfs_time_to_timespec(current_fs_time_sec(new_inode->i_sb));
	if (new_inode) {
		if (err)
			goto out;
		if (is_dir) {
