			...
			umode_t mode, bool excl)
{
	...
	dir_i->i_mtime = dir_i->i_ctime = timespec_to_vfs_time(ITIME(je32_to_cpu(ri->ctime)));
...
static int jffs2_unlink(struct inode *dir_i, struct dentry *dentry)
{
	...
	if (!ret)
		dir_i->i_mtime = dir_i->i_ctime = timespec_to_vfs_time(ITIME(now));
	return ret;
}
/***********************************************************************/
...
static int jffs2_symlink (struct inode *dir_i, struct dentry *dentry, const char *target)
{
	...
	*/
	...
	dir_i->i_mtime = dir_i->i_ctime = timespec_to_vfs_time(ITIME(je32_to_cpu(rd->mctime)));
			 ...
			 struct inode *new_dir_i, struct dentry *new_dentry)
{
	...
	*/
	...
	if (ret) {
		...
		new_dir_i->i_mtime = new_dir_i->i_ctime = timespec_to_vfs_time(ITIME(now));
		return ret;
	}
	...
	new_dir_i->i_mtime = new_dir_i->i_ctime = old_dir_i->i_mtime = old_dir_i->i_ctime = timespec_to_vfs_time(ITIME(now));
