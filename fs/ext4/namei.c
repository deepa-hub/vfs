			     ...
			     struct buffer_head *bh)
{
	 ...
	 */
	dir->i_mtime = dir->i_ctime = timespec_to_vfs_time(ext4_current_time(dir));
...
static int ext4_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	inode->i_ctime = dir->i_ctime = dir->i_mtime = timespec_to_vfs_time(ext4_current_time(inode));
	ext4_mark_inode_dirty(handle, inode);
...
static int ext4_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	if (retval)
		goto end_unlink;
	dir->i_ctime = dir->i_mtime = timespec_to_vfs_time(ext4_current_time(dir));
	ext4_update_dx_flag(dir);
	ext4_mark_inode_dirty(handle, dir);
	drop_nlink(inode);
	if (!inode->i_nlink)
		ext4_orphan_add(handle, inode);
	inode->i_ctime = timespec_to_vfs_time(ext4_current_time(inode));
	ext4_mark_inode_dirty(handle, inode);
		       ...
		       unsigned ino, unsigned file_type)
{
	...
	ent->dir->i_ctime = ent->dir->i_mtime =
		timespec_to_vfs_time(ext4_current_time(ent->dir));
		       ...
		       unsigned int flags)
{
	 ...
	 * rename.
	 */
	old.inode->i_ctime = timespec_to_vfs_time(ext4_current_time(old.inode));
	...
	if (new.inode) {
		...
		new.inode->i_ctime = timespec_to_vfs_time(ext4_current_time(new.inode));
	}
	old.dir->i_ctime = old.dir->i_mtime = timespec_to_vfs_time(ext4_current_time(old.dir));
