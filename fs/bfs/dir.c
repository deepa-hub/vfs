						...
						bool excl)
{
	...
	inode->i_mtime = inode->i_atime = inode->i_ctime = current_fs_time_sec(s);
	inode->i_blocks = 0;
						...
						struct dentry *new)
{
	...
	inc_nlink(inode);
	inode->i_ctime = current_fs_time_sec(s);
	mark_inode_dirty(inode);
	ihold(inode);
...
static int bfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	dir->i_ctime = dir->i_mtime = current_fs_time_sec(dir->i_sb);
			...
			struct inode *new_dir, struct dentry *new_dentry)
{
	...
	if (new_inode) {
		new_inode->i_ctime = current_fs_time_sec(new_dir->i_sb);
		...
		for (off = 0; off < BFS_BSIZE; off += BFS_DIRENT_SIZE) {
			...
			if (!de->ino) {
				...
				if (pos >= dir->i_size) {
					...
					dir->i_ctime = current_fs_time_sec(s);
				}
				dir->i_mtime = current_fs_time_sec(s);
				mark_inode_dirty(dir);
