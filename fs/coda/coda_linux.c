...
void coda_vattr_to_iattr(struct inode *inode, struct coda_vattr *attr)
{
	...
	if (attr->va_atime.tv_sec != -1) 
	        inode->i_atime = timespec_to_vfs_time(attr->va_atime);
	if (attr->va_mtime.tv_sec != -1)
	        inode->i_mtime = timespec_to_vfs_time(attr->va_mtime);
        if (attr->va_ctime.tv_sec != -1)
	        inode->i_ctime = timespec_to_vfs_time(attr->va_ctime);
}
...
void coda_iattr_to_vattr(struct iattr *iattr, struct coda_vattr *vattr)
{
	...
	vattr->va_rdev = -1;
        ...
        if ( valid & ATTR_ATIME ) {
                vattr->va_atime = vfs_time_to_timespec(iattr->ia_atime);
	}
        if ( valid & ATTR_MTIME ) {
                vattr->va_mtime = vfs_time_to_timespec(iattr->ia_mtime);
	}
        if ( valid & ATTR_CTIME ) {
                vattr->va_ctime = vfs_time_to_timespec(iattr->ia_ctime);
	}
}
